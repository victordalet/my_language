...
void min_array_int(char **int_array_name, int **int_array_values,
              ...
              char *line)
{
    ...
    for (int i = 0; i < int_array_count; i++) {
        if (!strcmp(var_name, int_array_name[i])) {
            int min = int_array_values[i][0];
            for (int j = 1; j < int_array_size[i]; j++) {
                if(int_array_values[i][j] <= min) {
                    min = int_array_values[i][j];
                }
            }
            printf("%d\n", min);
            break;
